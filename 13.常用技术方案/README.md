...
* 加载时性能优化
  * 资源小一点
    ...
    * css体积优化
    * 删除冗余代码、Tree Shaking
  * 请求少一点：应用初始化HTTP请求数从105个减少到56个，减少了47%，客户开发模块HTTP请求数从43个减少到32个，减少了 33%
    * 重复请求缓存结果
    * 非必要请求延迟加载
    * 可聚合请求接口改造
* 渲染时性能优化
...
#### 成果
* 内存：优化前内存 915MB，优化后减低了 131.2MB，减少了 14%。
* HTTP请求数：应用初始化HTTP请求数从105个减少到56个，减少了47%，客户开发模块HTTP请求数从43个减少到32个，减少了 33%。
* 长任务数：优化前人均次数87.03，优化后是68.31，减少了 21.5%。
* 路由切换耗时：新建自动开发任务路由切换耗时，P95从2.61S，减低到2.44S, 降低了0.17S。
* 组件重复渲染治理：新建自动开发任务从770次，减少到632次，减少了18%。
* js体积：从38.57MB减少到29.84MB，减少了22%的体积。
* css体积：优化前是2.5MB，优化后是2MB，减少了20%的体积。
* 用户侧：客服反馈崩溃卡顿案例从每月25条减少到6条，埋点系统统计的日崩溃率从0.78%减少到0.70%。

#### 数据驱动、指标先行
* 内存：利用 Electron 能力获取内容大小，并上报到埋点系统。优化前内存 915MB，优化后减低了 131.2MB，减少了 14%。
* HTTP请求数：api 层接口请求前统一处理，统计系统启动后15s/路由切换5s后，发起的请求数目。指标下降，意味着网路资源占用减少，不会阻塞关键接口的返回，避免页面长时间loading，同时也可以减少页面的无效渲染。
* 长任务数：new PerformanceObserver 统计大于 50ms 的任务会被记录为长任务，并上报到埋点系统。优化后长任务减少了 27.94%。
* 路由切换耗时：使用 Sentry 的指标，统计路由切换耗时，P75从2.61S，减低到2.44S, 降低了 0.17S。
* LCP/FCP时间：使用 Sentry 的指标，统计页面FCP（首次渲染时间：第一个内容元素对用户可见）LCP（主要内容何时对用户可见：页面中最大的可见元素在视窗内完全渲染出来的时间点）。

#### HTTP请求治理：应用初始化HTTP请求数从105个减少到56个，减少了47%，客户开发模块HTTP请求数从43个减少到32个，减少了 33%
...
#### 组件重复渲染治理：使用Chrome工具React DevTools统计了几个关键路径上渲染次数，新建自动开发任务从770次，减少到632次，减少了18%
...
#### js拆分：使用 webpack-bundle-analyzer 分析构建后的js产物，找出值得优化的问题，最后js体积从38.57MB减少到29.84MB，减少了22%的体积
一共减少了15MB的js体积
...
#### css 体积优化：优化前是2.5MB，优化后是2MB，减少了20%的体积
css 现状存在的问题
...
#### 后续可优化手段
* React重复渲染优化效果不明显，后期对于不规范的React开发习惯的约束，可以考虑阻止一些最佳实践的宣讲，以及定期的 CodeReview
* 实现状态快照功能，支持模块释放和还原，减少内存占用
* 引入 tailwindcss，利用原子化css的能力，减少css体积，提升开发效率
* 开发阶段和后端同学多关注接口拆分问题，可以通过聚合单个接口，提升性能
...
## 为什么要使用Electron，使用了Electron的哪些功能？
Electron 是一个框架，使开发者能够将 Web 技术(HTML、JavaScript、CSS)、Node.js 及原生代码相结合，构建适用于 macOS、Windows 和 Linux 的跨平台桌面应用程序。开发桌面端应用的框架，还有 Tarui、NW.js、Flutter。但是 Tauri 需要学习 Rust，Flutter 需要学习 Dart，学习成本高，所有没有选择。NW.js 虽然类似 Electron，也是使用 HTML、CSS、JavaScript，但是用户群体小，不如 Electron。但是 Electron 也有着天然的短板，比如：Electron 相比其他 Native 来说，包体积和 CPU 资源占用较高；我们还需要额外解决一些安全性问题和性能体验问题。

#### 为什么要开发桌面端而不是直接使用web端呢
桌面端有以下优势，所以用户体验好，更容易推广，也可以提高用户留存
* 无兼容性问题，Electron 内置了 Chromium 浏览器。并且，只需要升级Electron版本，就可以使用浏览器最新的 Feature
* 文件系统访问：可以直接读写、创建、删除用户本地文件，而不需要通过<input type="file">上传这种笨拙的方式
* 客户端 GUI：右键菜单、窗⼝定制、系统托盘
* 桌⾯环境集成：系统通知、剪切板、系统快捷键

#### 使用到的Electron的功能
* appManage
  * 开机自启动：auto-launch
  * 复制文本：clipboard.writeText(text)，相比较直接js使用navigatornavigator.clipboard.writeText(text) 有更高的权限，更好的兼容性
  * 创建应用菜单：Menu.buildFromTemplate(menuConf)
  * 系统托盘；new Tray(this.getTrayIcon())
  * 全局快捷键：globalShortcut
  * 打印：webContents.printToPDF()，相比较web端的window.print()调用浏览亲打印来说，Electron可以自定义打印样式
* downloadManage
  * 删除文件：fse.remove(filePath)
  * 判断文件是否存在：fse.pathExistsSync(path)
  * 移动文件：fse.move(tempFilePath, filePath)
* windowManage
  * 获取窗⼝：BrowserWindow.getAllWindows()
  * 创建窗口：new BrowserWindow(windowOps)
